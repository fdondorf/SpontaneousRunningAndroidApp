...
public class DatabaseHelper extends SQLiteOpenHelper {
   ...
   private final static String TAG = "SpontaneousRunning.DatabaseHelper";
   ...
   /**
    * Will update version 1 through 2 to version 3
    * 
    ...
    */
   @Override
   public void onUpgrade(SQLiteDatabase db, int current, int target) {
      Log.i(TAG, "Upgrading " + current + " to " + target);
      ...
      if (current == 2) { // From 2 to 3 ( Set user to "1" for all given tracks )
         for (String statement : GPSTracking.Tracks.UPGRADE_STATEMENT_2_TO_3) {
            db.execSQL(statement);
         }
         current = 3;
      }
...
//      }
    ...
    * @return
    */
   long insertWaypoint(long trackId, long segmentId, Location location, Double distance) {
      if (trackId < 0 || segmentId < 0) {
         throw new IllegalArgumentException("Track and segments may not the less then 0.");
      }
      ...
      args.put(WaypointsColumns.DISTANCE, distance);
...
//            new String[] { String.valueOf(trackId), String.valueOf(segmentId) });
   ...
   int updateTrack(long trackId, String name, String totalDistance, Long totalDuration) {
      int updates;
      String whereclause = Tracks._ID + " = " + trackId;
      ContentValues args = new ContentValues();
      args.put(Tracks.NAME, name);
      args.put(Tracks.TOTAL_DISTANCE, totalDistance);
      args.put(Tracks.TOTAL_DURATION, totalDuration);
   ...
   int updateSegment(long segmentId, Long endTime) {
	      int updates;
	      String whereclause = Segments._ID + " = " + segmentId;
	      ContentValues args = new ContentValues();
	      args.put(Segments.END_TIME, endTime);
	      
	      // Execute the query.
	      SQLiteDatabase mDb = getWritableDatabase();
	      updates = mDb.update(Segments.TABLE, args, whereclause, null);

	      ContentResolver resolver = this.mContext.getContentResolver();
	      Uri notifyUri = ContentUris.withAppendedId(Segments.CONTENT_URI, segmentId);
	      resolver.notifyChange(notifyUri, null);
   ...
   long toNextSegment(long trackId) {
      ...
      args.put(Segments.START_TIME, System.currentTimeMillis());
      args.put(Segments.END_TIME, System.currentTimeMillis());
